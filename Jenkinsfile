  ...
  stages {
    ...
    stage('Package and Push Helm Chart') {
      steps {
        ...
        mysh "docker run --rm -v \$(pwd)/helm_repo:/apps google/cloud-sdk gcloud auth activate-service-account --project=rl-global-eu --key-file=/key.json && gsutil -m rsync -r /apps/ gs://rl-helm"
      }
    }
