  ...
  stages {
    ...
    stage('Package and Push Helm Chart') {
      steps {
        ...
        mysh "docker run --rm -v /var/lib/jenkins/.gcp/key.json:/key.json -v \${WORKSPACE}/helm_repo:/mnt google/cloud-sdk \"gcloud auth activate-service-account --project=rl-global-eu --key-file=/key.json && gsutil -m rsync -r /mnt/ gs://rl-helm\""
      }
    }
